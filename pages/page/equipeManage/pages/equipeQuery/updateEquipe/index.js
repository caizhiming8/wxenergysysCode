// pages/page/equipeManage/pages/equipeQuery/updateEquipe/index.js
const app = getApp();
Page({
  ...
  onLoad: function (options) {
    ...
    data.reqData.device_type = cur_info.device_type;
    data.reqData.id = cur_info.id;
    data.reqData.inst_position = cur_info.inst_position;
    data.reqData.assets_attribution = cur_info.assets_attribution;
    data.reqData.phone = cur_info.phone;
    data.reqData.system_id = cur_info.system_id;
    data.reqData.subsystem_id = cur_info.subsystem_id ? cur_info.subsystem_id: '';
    data.reqData.person_liable = cur_info.person_liable;
    _this.setData({
      userPhone: cur_info.phone,
      assets_attribution: cur_info.assets_attribution
    })
      ...
      .then(res => {
        if (res.success) {
          ...
          res.result.forEach((item,index)=> {
            if (item.systemId == cur_info.system_id) {
              _this.setData({
                cur_system: index
              })
              if(cur_info.subsystem_id) {
                _this.setData({
                  showSubSystem: true
                })
                if (item.children) {
                  if (item.children[0].name != '选择归属子系统') {
                  ...
                  _this.setData({
                    'subSystemArrList': item.children
                  })
                  item.children.forEach((list,index2)=> {
                    if (list.subsystemId == cur_info.subsystem_id) {
                      _this.setData({
                        cur_subSystem: index2
                      })
                    }
                  })
                }
      ...
      .then(res => {
        if (res.success) {
          ...
          res.result.list.forEach((item, index) => {
            if (item.id == cur_info.person_liable) {
              _this.setData({
                cur_person: index
              })
            }
    ...
    app.globalObj.apiConfig.getAllDeviceList()
      .then(res => {
        if (res.success) {
          ...
          res.result.deviceTypeList.forEach((item, index) => {
            if (item.device_type == cur_info.device_type) {
              _this.setData({
                cur_deviceType: index
              })
            }
          })
        } else {
  ...
  getRoomDataList() {
    let _this = this
      , data = this.data;
    app.globalObj.apiConfig.goShowToast('加载中', 'loading', 15000);
    app.globalObj.apiConfig.getAllRoomsList({
      building_id: data.reqData.building_id
  ...
  goSubmitForm(e) {
    ...
    app.globalObj.apiConfig.goShowToast('加载中', 'loading', 15000);
    app.globalObj.apiConfig.getEquipeUpdateEquipe(cur_obj)
      .then(res => {
        if (res.success) {
          app.globalObj.apiConfig.goShowToast('更新成功', 'none', 2000, false);
          setTimeout(() => {
            wx.navigateBack({
              detal: 1
            })

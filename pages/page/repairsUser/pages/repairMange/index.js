...
Page({
  ...
  goChangeTab(e) {
    ...
    if (cur_index == 1) {
      ...
      delete _this.data.reqData.status;
    }
    _this.setData({
  ...
  goLookDetail(e) {
    ...
    if (_this.data.cur_activeTab == 0) {
      if (cur_info.status == 1) {
        // 指派维修人员
        wx.navigateTo({
          url: `./repairsDetail/index?id=${JSON.stringify(cur_info)}&type=1`
        })
      } else if (cur_info.status == 2) {
        // 修改维修人员
        wx.navigateTo({
          url: `./delegateDetail/index?id=${JSON.stringify(cur_info)}&type=2`
        })
  ...
  goAssignedPersonnel(e) {
    ...
    if (_this.data.cur_activeTab == 0) {
      ...
      } else if (cur_info.state == 2) {
        // 修改维修人员
        wx.navigateTo({
          url: `./assignedPerson/index?id=${JSON.stringify(cur_info)}&type=2`
        })
      } else if (cur_info.state == 3) {
        // 查看进度
        wx.navigateTo({
          url: `./lookPlan/index?id=${JSON.stringify(cur_info)}`
        })
      } else if (cur_info.state == 4) {
        // 查看评论
        wx.navigateTo({
          url: `./lookComment/index?id=${JSON.stringify(cur_info)}`
        })
      ...
      // 填写评价
      wx.navigateTo({
        url: `./appraiseView/index?id=${JSON.stringify(cur_info)}`
      })
    } else if (_this.data.cur_activeTab == 4) {
      // 查看评论
      wx.navigateTo({

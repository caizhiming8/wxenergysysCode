// pages/page/message/pages/index.js
const app = getApp()
Page({
  ...
  onLoad: function (options) {
    ...
    if(options.options) {
      ...
      const firstPromise = new Promise((rej)=> {
        app.globalObj.apiConfig.getChangeSystem({ 
          'project_id': options.project_id,'login_name': options.login_name,'info_flag':1 })
          .then(res => {
            if (res.success) {
              app.globalObj.apiConfig.goShowToast('切换成功', 'none', 2000, false);
              rej();
            } else {
  ...
  getInfoData() {
    ...
    if (data.activeNav == 1) {
      ...
      // 我的消息
      app.globalObj.apiConfig.getMessageList_send({ projectId: app.globalUserData.User_Info.preprojectId,login_name: app.globalUserData.User_Info.loginName})
        .then(res => {
          if (res.success) {
            app.globalObj.apiConfig.goHideToast();
            res.result.messageList.forEach(item=> {
              if(item.status == 'touser字段openid为空或者不正确') {
                item.status = '用户未绑定小程序';
              }
            })
            _this.setData({
              'responceData': res.result.messageList,
